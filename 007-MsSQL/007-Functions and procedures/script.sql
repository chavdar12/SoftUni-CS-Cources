...
CREATE
PROC usp_GetEmployeesSalaryAbove35000 AS
SELECT FirstName, LastName
FROM Employees
WHERE Salary > 35000
    - -2. Employees
with Salary Above Number
CREATE
PROC usp_GetEmployeesSalaryAboveNumber(@Salary DECIMAL(18,4)) AS
SELECT FirstName, LastName
FROM Employees
WHERE Salary >= @Salary
    - -3. Town Names Starting
With
CREATE
PROC usp_GetTownsStartingWith(@Start NVARCHAR(20)) AS
SELECT Name AS Town
FROM Towns
...
BEGIN DECLARE @CurrentSalary NVARCHAR(25)
SET @CurrentSalary =
        CASE
            WHEN @Salary < 30000 THEN 'Low'
            WHEN @Salary <= 50000 THEN 'Average'
            ELSE 'High'
            END RETURN @CurrentSalary
END
...
BEGIN DECLARE @Result DECIMAL(20,4) = @Sum * POWER((1+@YearlyInterest), @Years)
	RETURN @Result
END
...
CREATE
PROC usp_CalculateFutureValueForAccount (@AccID INT, @InterestRate FLOAT)
AS
...
FROM AccountHolders AS ah
    JOIN Accounts AS a
ON a.AccountHolderId=ah.Id AND a.Id = @AccID
...
FROM (SELECT ug.Cash, ROW_NUMBER() OVER (ORDER BY Cash DESC) AS RowNumber
      FROM UsersGames AS ug
               JOIN Games AS g ON ug.GameId = g.Id
      WHERE g.Name = @GameName) AS AllGames
WHERE RowNumber % 2 = 1
    - -
SELECT *
