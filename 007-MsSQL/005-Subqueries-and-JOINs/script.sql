...
FROM (SELECT AVG(Salary) AS MinAverageSalary
      FROM Employees
...
FROM (SELECT ContinentCode,
             CurrencyCode, [CurrencyCount], DENSE_RANK () OVER
...
--			GROUP BY ContinentCode, CurrencyCode) AS CurrencyCountQuery)
      ...
      WHERE mc.MountainId IS NULL
          --Problem 17. Highest Peak
        and Longest River by Country
      SELECT TOP(5)
      ...
      SELECT *, DENSE_RANK () OVER (PARTITION BY [Country] ORDER BY [Highest Peak Name] DESC) AS DRank
      INTO #TZ FROM
      (SELECT
