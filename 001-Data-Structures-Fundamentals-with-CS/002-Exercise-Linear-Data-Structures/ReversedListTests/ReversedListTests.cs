...
public class ListTests
{
    ...
    public void RemoveShouldWorkCorrectly()
    {
        ...
        var randomIndex = _random.Next(array.Length) + 1;
    ...
    public void InsertShouldThrowExceptionIfInvalidIndexIsPassed()
    {
        ...
        for (var i = 0; i < count; i++)
        {
            Assert.Throws<IndexOutOfRangeException>(() => { list.Insert(-(1 + i), _random.Next(1, 5)); });
            Assert.Throws<IndexOutOfRangeException>(() => { list.Insert(count + i, _random.Next(1, 5)); });
        }
    }

    private static IAbstractList<int> GetList()
    {
        return new ReversedList<int>();
    }
}